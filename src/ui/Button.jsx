...
function Button({ children, disabled, to, type, onClick }) {
    ...
    const styles = {
        ...
        round: base + ' px-2 py-1 md:px-2 md:py-.5 text-xs',
        secondary:
            'inline-block text-sm rounded-full border-2 broder-stone-300 font-semibold uppercase tracking-wide text-stone-800 transition-colors duration-300 hover:bg-stone-300 hover:text-stone-800 focus:text-stone-800 foucs:bg-stone-300 focus:outline-none focus:ring focus:ring-stone-200 focus:ring-offset-2 disabled:cursor-not-allowed px-4 py-2.5 md:px-6 md:py-3.5',
    };
    ...
    if (onClick) {
        return (
            <div className={styles[type]} disabled={disabled} onClick={onClick}>
                {children}
            </div>
        );
    }
